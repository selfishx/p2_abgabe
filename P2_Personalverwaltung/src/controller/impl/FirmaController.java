...
public class FirmaController extends AbstractController implements ActionListener {
	...
	/**
	 * Konstruktor des Controller. Es werden das Model und die View dem Controller
	 * als Parameter übergeben. Der Controller setzt sich innerhalb der View mittels
	 * der Funktion FirmaView.setController als Verantwortlicher für die
	 * auszuführende Logik.
	 * 
	 * @param firma     Model
	...
	/**
	 * Diese Methode wird vom Interface <tt>ActionListener</tt> bereitgestellt und
	 * muss in der konkreten Umsetzung implementiert werden. Im konkreten Fall
	 * werden den vom Typ <tt>InterfaceView</tt> vererbten Views Möglichkeiten
	 * bereitgestellt, Actions innerhalb der View an den Controller zu delegieren.
	 * 
	...
	public void actionPerformed(ActionEvent event) {
			...
			if (datei != null) {
				...
				try {
					...
					JOptionPane.showMessageDialog(new JFrame(),
							"Es trat ein Fehler beim Auswerten der Daten auf. Abbruch!", "Fehler", 0);
					return;
				} catch (IOException e) {
					JOptionPane.showMessageDialog(new JFrame(),
							"Es trat ein Fehler beim Einlesen der Datei auf. Abbruch!", "Fehler", 0);
					return;
				}
			}
		}

		if (this.firmaView != null && actionCommand == this.firmaView.getBtnSpeichern().getText()
				&& this.firmaModel.isDirty()) {
			...
			if (datei != null) {
				try {
					...
					JOptionPane.showMessageDialog(new JFrame(),
							"Es trat ein Fehler beim Speichern der Datei auf. Abbruch!", "Fehler", 0);
					return;
				}
			}
		}
		...
		// Button zum Hinzufügen eines neuen Angestellten im Dialog neuen Mitarbeiter
		// hinzufügen
	...
	/**
	 * Einlesen einer komma-separierten Datei (CSV) mit entsprechenden
	 * Informationen. Jede Zeile muss mit dem Kenner "objekt" beginnen
	 * 
	 * @param datei
		...
		try {
			...
			if (zeile.equals("gehälter")) {
				...
				while ((zeile = dateiEinlesen.readLine()) != null) {
					...
					// Eingelesene Zeilen in die Gehaltsliste (Array gehaltsliste im FirmaModel)
					// übertragen
	 ...
	 */
	public void createNewAngestellten(String angestellter, String vorname, String nachname, String geburtsdatum,
			String geschlecht, String telefon, String angestelltenNr, String gehaltsgruppe, String erfahrungsstufe) {
	...
	public void angestellterHinzufuegen(AngestellterModel model) {
		...
		// Prüfen, ob ein evtl. vorhandenes Objekt aktualisiert wurde. Dann an dieser
		// Stelle in der Liste
