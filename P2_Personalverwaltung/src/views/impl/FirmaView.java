...
/**
 * Die FirmaView stellt alle Informationen der Software gebündelt dar. Sie ist
 * die Hauptdarstellungsfläche der Anwendung, auf der alle Komponenten
 * positioniert sind. Die FirmaView enthält eine ToolBar, eine SplitPane sowie
 * innerhalb der SplitPane eine Tabelle (links) und eine JPane mit weiteren
 * Elementen zur Darstellung von Inhalten (rechts). Die FirmaView meldet sich
 * als Beobachter bei dem FirmaModel sowie beim aktuell angezeigten
...
public class FirmaView extends JFrame implements Observer, InterfaceView {
	...
	// Wert zur Serialisierung von SWING-Objketen. Hier nur, um Compiler-WARNING zu
	// unterdrücken!
	...
	private JButton btnSpeicherNeuenAngestellten = new JButton("Hinzufügen");// Neuen mitarbeiter hinzufügen im Dialog
																				// neuer Angestellter
	private JButton btnAbbrechen = new JButton("Abbrechen");// Aktion Mitarbeiter hinzufügen abbrechen im Dialog neuer
															// Angestellter
	private JButton btnEntfernen = new JButton("Entfernen");// Aktion Mitarbeiter entfernen bestätigen im Dialog
															// Angestellten entfernen
	private JButton btnBestätigen = new JButton("Bestätigen");// Aktion Mitarbeiter bearbeiten bestätigen im Dialog
																// Angestellten bearbeiten

	/*
	 * Liste aller Angestellten, die bei einem EVENT update befüllt wird Die Liste
	 * wird der JList listAngestellteListe übergeben
	 */
	...
	/**
	 * Constructor. Erzeugt die FirmaView und setzt das Model. Dem Model wird als
	 * Beobachter die View hinzugefügt, d.h. bei Änderungen am Model wird die View
	...
	private void createContent() {
		...
		/*
		 * Setzen des Fenster-Titels, Aktivieren des roten X (oben rechts) zum Fenster
		 * schließen Größe des Fensters und Startpunkt (X,Y) auf dem Bildschirm
		 * definieren
		 */
		...
		/*
		 * Haupt-Zeichenfläche (JPane-Komponente) des Fensters, auf dem alle anderen
		 * Komponenten hinzugefügt werden.
		 * 
		 * Setzen eines leeren Rahmens (EmptyBorder) mit 5px Abstand zu allen Seiten,
		 * Hinzufügen der JPane-Komponente zum Fenster, Setzen des BorderLayouts zur
		 * Ausrichtung der Komponenten auf der JPane
		 */
		 ...
		 * Links befindet sich die tabellarische Darstellung aller Angestellten, rechts
		 * Informationen zu einem selektierten Angestellten
		 */
		JSplitPane splitPane = new JSplitPane();
		contentPane.add(splitPane, BorderLayout.CENTER);
		splitPane.setDividerLocation(250);

		// Liste "Angestellte" erstellen. Sie dient zur Anzeige im Auswahlfenster der
		// Mitarbeiter (das Fenster auf der linken Seite)
		...
		/*
		 * SelectionListener der Angestellte-Tabelle. Dieser Listener reagiert auf
		 * Veränderungen der selektierten Elemente innerhalb der Tabelle. Wird bspw. ein
		 * Eintrag der Tabelle selektiert, wird dieser Listener informiert.
		 * 
		 * Der SelectionListener ist als anonyme innere Klasse umgesetzt.
		 */
		this.listAngestellteListe.getSelectionModel().addListSelectionListener(new ListSelectionListener() {
			@Override
			public void valueChanged(ListSelectionEvent event) {

				// Wenn ein Eintrag in der Termin-Tabelle selektiert wird und es sich dabei um
				// das
				// letzte Event der Eventkette von Swing (valueIsAdjusting) handelt, wird der
				// "Termin Bearbeiten"-Button und der "Angestellten entfernen"-Button aktiviert
				// und die Variable gewaehlterTermin mit dem
				...
				if (FirmaView.this.listAngestellteListe.getSelectedIndex() > -1 && !event.getValueIsAdjusting()) {
					 ...
					 * (AngestellteNr) ausgelesen Eine Schleife zählt die nicht gelöschten
					 * Mitarbeiter in der ArrayList angestellte (FirmaModel). Für jeden gelöschten
					 * Mitarbeiter (Telefonnummer wird überprüft) wird der Wert "gelöschte" um eins
					 * erhöht. Die Schleife läuft solange, bis die Anzahl der nicht gelöschten
					 * Mitarbeiter = des Wertes getSelctedIndex() ist. Der Wert gelöschte +
					 * getSelectedIndex() wird dannn im Wert angestellter gespeichert.
					 */
					...
					// Sobald ein Eintrag ausgewählt wird, wird das Gehalt des Angestellten
					// berechnet und geladen
					...
					FirmaView.this.lblGeschlechtWert
							.setText((angestellter.getGeschlecht() == 0) ? "Männlich" : "Weiblich");
		...
		// Die Informationen liegwn innerhalb einer JScrollPane-Komponente, damit, wenn
		// mehr
		// Einträge als darstellbare Fläche vorhanden ist, Bildlaufleisten angezeigt
		// werden.
		...
		// Es wird ein Absolute Layout verwendet. Deswegen muss kein explizites Layout
		// gesetzt werden!
	...
	public void showAddMitarbeiterWindow() {
		...
		txtAngestelltenNummer.setEditable(false); // Verhindert das Bearbeiten des Feldes AngestelltenNr. Die Nummer
													// wird automatisch generiert.

		// Arrays für die Comboboxes deklarieren und initialisieren
		String[] tage = { "01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12", "13", "14", "15",
		...
		txtTelefon.setText("");
		txtAngestelltenNummer.setText(Integer.toString(model.getAngestellteListe().size() + 1)); // generiert eine neue
																									// Nummer für den
																									// Mitarbeiter. Sie
																									// ist fortlaufend,
																									// basierend auf
																									// Länge
																									// model.getAngestellteListe().size()
		...
		txtTelefon.addKeyListener(new KeyListener() {
			...
			public void keyTyped(KeyEvent e) {
				...
				/*
				 * If else zur Überprüfung, ob etwas eingegeben wurde. Vorher wird der
				 * Hinzufügen-Butten false gesetzt. Es müssen alle Felder befüllt sein
				 */
	 ...
	 ** Übergibt die Parameter (angestellter, vorname, nachname, geburtsdatum,
	 * geschlecht, telefon, angestelltenNr, gehaltsgruppe, erfahrungsstufe) indem
	 * die Methode createNewAngestellter der Klasse Controller aufegerufen wird
	 */
	public void addNewAngestellter() {

		geburtsdatumNewAngestellter = "" + boxTag.getSelectedItem() + "." + boxMonat.getSelectedItem() + "."
				+ boxJahr.getSelectedItem();
		...
		// Aus dem Geschlecht eine Zahl im Stringformat generieren um diese
		// ordnungsgemäß als Parameter zu übergeben
		if (geschlechtNr == 0) {
			geschlecht = "0";
		} else {
			geschlecht = "1";
		...
		controller.createNewAngestellten(angestellter, vorname, nachname, geburtsdatum, geschlecht, telefon,
				angestelltenNr, gehaltsgruppe, erfahrungsstufe);
		removeListener();
	}

	/**
	 * Ruft ein "Sicher löschen?"-Fenster beim klicken von "Angestellten entfernen"
	 * auf
	 * 
	 */
	...
	public void showAngestelltenBearbeiten() {
		...
		// Geburtstag und Geschlecht sollen nicht editierbar sein, da solche Änderungen
		// häufiger ungewollt als gewollt passieren...
	...
	/**
	 * Liefert den derzeit in der Views selektierten und dargestellten Angestellten
	 * respektive sein Datenobjekt
	 * 
	...
	/**
	 * Diese Methode setzt alle Anzeigefelder der Angestelltendaten auf "leer"
	 * zurück.
	 * 
	...
	/**
	 * Diese Methode entfernt alle Angestellten aus der Angestelltentabelle. Dies
	 * ist z. B. notwendig, wenn eine neue Datei geöffnet wird, um alle zuvor
	 * dargestellten Daten zu entfernen.
	 */
	...
	/**
	 * Setzt den Controller zur Delegation der Aufgaben. Er muss vom Typ
	 * FirmaController sein
	 * 
	...
	/**
	 * Diese Methode wird durch das Implementieren des Observer-Interfaces
	 * bereitgestellt. Meldet sich die View bei anderen, observierbaren Objekten als
	 * Beobachter an, so rufen diese - beim Eintreten eines Ereignisses - diese
	 * Methode auf, um die View über Änderungen zu informieren.
	 * 
	...
	public void update(Observable quelle, Object daten) {
		...
		/*
		 * Prüfung, ob die Variable daten eine bestimmte Objektstruktur aufweist. Es
		 * wird davon ausgegangen, dass daten Event-Informationen beinhaltet und daher
		 * vom Typ eine konkrete Implementierung von AbstractEvent ist.
		 */
		if (daten instanceof FirmaDataEvent) {
			...
			/*
			 * FirmaAngestellterEvent. Es liegen veränderte AngestellterModel-Informationen
			 * vor. Das Event kann unterschiedliche Formen des EventType annehmen. Im
			 * folgenden Fall werden ADD (Hinzufügen) und REMOVE (Entfernen) unterstützt
			 */
			FirmaAngestellterEvent ppe = (FirmaAngestellterEvent) daten;
			if (ppe.getType() == EventType.ADD) {
				// Angestellter in die Angestellten-Liste (die auf der linken Seite angezeigt
				// wird) aufnehmen

				mitarbeiter.removeAllElements();

				// Überprüft ob es sich um einen gelöschten Eintrag handelt und übergibt das
				// Objekt an die Mitarbeiterliste
			...
			/*
			 * AngestellterDataEvent. Es liegen veränderte AngestellterModel-Informationen
			 * für den derzeit dargestellten Angestellten vor. Es werden alle Felder der
			 * View mit den AngestellterModel- Daten neu befüllt
			 */
	 ...
	 * und öffnet dann das Programmfenster (JFrame) mittels der Swing EventQueue
	 * (Threading).
	 * 
	...
	/**
	 * Setzt den Actionlistener der Buttons btnSpeicherNeuenAngestellten und
	 * btnAbbrechen zurück
	 * 
	 */
